...
function CDB() {
  ...
  const [ir, setIr] = useState("R$ 0,00")
  ...
  const calculate = () =>{
    ...
    let cdiMonth = Math.pow(1 + cdi/100, 1/12) - 1
    ...
    // Cálculo de IR
    let ir = 0
    if (t <= 6) ir = (M - (C + (A ? A * t : 0)))*0.225
    else if (t <= 12) ir = (M - (C + (A ? A * t : 0)))*0.2
    else if (t <= 24) ir = (M - (C + (A ? A * t : 0)))*0.175
    else ir = (M - (C + (A ? A * t : 0)))*0.15

    setIr(Intl.NumberFormat('pt-BR', {style: 'currency', currency: 'BRL'}).format(ir))
  ...
  return (
    <form className="mx-auto mt-10 w-[350px] bg-white rounded-lg shadow-lg border-2 border-gray-200">
      <h2 className="mt-3 text-xl font-bold mb-3">Calculadora de CDB</h2>
      ...
      <Modal result={result} fees={fees} accumulated={accumulated} ir={ir}/>
    </form>
  );
}
